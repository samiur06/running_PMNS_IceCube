...
Released with [arXiv:2511.07541](https://arxiv.org/abs/2511.07541) - "_Double Bangs in IceCube as a Window to the Neutrino Mass Origin_". 
...
### Overview  
Stores the benchmarks, parameter points, event differences, and the plotting script with details below.

The free parameters in this model are those that can be chosen randomly at the production scale (also, see their descriptions in our paper [arXiv:2511.07541](https://arxiv.org/abs/2511.07541)). These are the atmospheric parameters $\{ \theta_{23}, \Delta m^2_{31}\}$, the CP-odd phases ($\tilde{\alpha}(Q_p^2), \tilde{\beta}(Q_p^2)$, and $\delta(Q_p^2)$), the lightest neutrino mass $m_\nu$ (shown as $m_1$ since we consider normal ordering), the angles parameterizing the orthogonal $R$ matrix $\{\xi_i\}$, and  the Yukawa matrix $Y_N$, i.e., $\text{diag}(Y_N ^{(1)},Y_N ^{(2)},Y_N ^{(3)})$. $R$ is taken to be a real matrix and parameterized by three Euler-like rotation angles $\xi_1$ in the 1-2 plane, $\xi_2$ in the 2-3 plane, and $\xi_3$ in the 1-3 plane. Each 'parameter point' corresponds a set of these 12 free parameters, namely    
{ $\{\tilde{\alpha},\tilde{\beta},\delta,\Delta m^2_{31},\sin^2 \theta_{23},\xi_{1},\xi_{2},\xi_{3}, (Y_N ^{(1)},Y_N ^{(2)},Y_N ^{(3)}),m_1\}$ }.  
